...
class meu_robo:
    def __init__(self, r, d, dt, int_pose):
    ...
    def calcular_pos(self, wL, wR, phi, delta_t, metodo):

        v_k = (self.r / 2) * (wL + wR)
        w_k = (v_k * math.tan(phi)) / self.d
...
def main():
    ...
    comandos = [
        dict(wL=20.0, wR=20.0, phi= 0.0,    delta_t=3.0),
        dict(wL=20.0, wR=20.0, phi= 0.3491, delta_t=2.0),
        dict(wL=20.0, wR=20.0, phi= 0.0,    delta_t=3.0),
        dict(wL=20.0, wR=20.0, phi=-0.3491, delta_t=2.0),
    ]
    ...
    robo_exata = meu_robo(r, d, dt, pose_inicial.copy())
    ...
    robo_euler = meu_robo(r, d, dt, pose_inicial.copy())
    ...
    robo_pm = meu_robo(r, d, dt, pose_inicial.copy())
    ...
    gt = np.genfromtxt("teoria_robotica/lista-8/ground_truth_ackermann.csv", delimiter=",", skip_header=1)
    gt_x = gt[:, 1]
    gt_y = gt[:, 2]
